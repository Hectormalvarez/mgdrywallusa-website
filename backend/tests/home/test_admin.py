...
def test_field_has_help_text(model_cls, field_name, expected_substring):
    ...
    assert field.help_text, f"{model_cls.__name__}.{field_name} is missing help_text"
    assert expected_substring.lower() in field.help_text.lower(), (
        f"{model_cls.__name__}.{field_name} help_text should mention '{expected_substring}' — got: {field.help_text}"
    )
...
def test_icon_help_text_lists_valid_values():
    ...
    for valid_icon in ("wall", "patch", "paint", "shield"):
        assert valid_icon in field.help_text, f"icon help_text must list '{valid_icon}' as a valid value"
    # "building" was previously listed but is NOT a supported frontend icon
    assert "building" not in field.help_text, "icon help_text must not list 'building' — it is not a valid icon"
