...
US_PHONE_RE = re.compile(r"^(\+1[\s\-]?)?\(?\d{3}\)?[\s\-]?\d{3,4}[\s\-]?\d{0,4}$")
...
class LeadSerializer(serializers.Serializer):
    ...
    def validate_phone(self, value: str) -> str:
        if not US_PHONE_RE.match(value):
            raise serializers.ValidationError("Enter a valid US phone number (e.g. 555-123-4567).")
        return value

    def validate_photos(self, files: list) -> list:
        if len(files) > MAX_FILES:
            raise serializers.ValidationError(f"No more than {MAX_FILES} files allowed.")
        total = 0
        for f in files:
            if f.content_type not in VALID_PHOTO_MIME_TYPES:
                raise serializers.ValidationError(
                    f"'{f.name}' is not a supported image type. Allowed types: JPEG, PNG, WebP."
                )
            ...
            if ext not in VALID_PHOTO_EXTENSIONS:
                raise serializers.ValidationError(
                    f"'{f.name}' has an invalid file extension. Allowed extensions: .jpg, .jpeg, .png, .webp."
                )
    ...
    def validate(self, data):
        ...
        if data.get(HONEYPOT_FIELD):
            raise serializers.ValidationError({HONEYPOT_FIELD: "spam detected"})
        return data
