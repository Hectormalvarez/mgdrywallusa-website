...
**US-010 D′ cutover COMPLETE (2026-09-16 15:19) — deploys now run over Cloudflare-Access-protected SSH from GitHub Actions; webhook daemon retired.** Release job: pinned cloudflared → `access tcp --hostname ssh.taylormadetech.net` (service token) → `ssh -p 2222 hadev@localhost deploy sha-<sha>` → forced-command `scripts/deploy-wrapper.sh` → deploy.sh (sync, exit code = truth). Access app `ssh-usrv01` policy `non_identity` — browser SSH closed. Legacy webhook container removed; prod tunnel ingress now just site+404. First D′ deploy: green in 54s, images sha-c03af50, site 200/HIT. Zero-Trust hardening same run: code-server service disabled, pgadmin/postgres containers removed (data backed up), code./pgadmin. ingress+Access apps deleted, stale /etc/cloudflared/config.yml trimmed. **Open:** revoke ZT token (user dashboard), delete dead code./pgadmin. CNAMEs (need DNS:Edit), optional /opt symlink removal. CF creds convention: `~/.cloudflare/tokens.env` (source with `set -a; . ~/.cloudflare/tokens.env; set +a`).
